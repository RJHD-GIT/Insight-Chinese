...
#define GDBTK_NOTIFIER_BLOCK_GDB_POLL	(1 << 2) /* Block gdb events polling. */
...
gdbtk_notifier_schedule_proc (gdb_client_data clientData)
{
  ...
  clear_async_event_handler (notif->schedule);
  ...
  if (timeptr)
    {
      ...
      if (!msec && (notif->flags & GDBTK_NOTIFIER_BLOCK_GDB_POLL))
        return 0;
    }
...
/* Allow reentering Tcl from the gdb event loop. */
void
gdbtk_reenter_tcl (void)
{
  gdbtk_notifier_state *notif = notifier_state ();
...
/* Block gdb events polling. */
bool
gdbtk_block_gdb_events_polling (bool block)
{
  gdbtk_notifier_state *notif = notifier_state ();
  bool previous = !!(notif->flags & GDBTK_NOTIFIER_BLOCK_GDB_POLL);

  notif->flags &= ~GDBTK_NOTIFIER_BLOCK_GDB_POLL;
  if (block)
    notif->flags |= GDBTK_NOTIFIER_BLOCK_GDB_POLL;
  return previous;
}
...
gdbtk_start_timer (void)
{
  ...
  if (target_should_use_timer (current_inferior ()->top_target ()))
    {
...
_initialize_gdbtk (void)
{
  ...
  if (strcmp (interpreter_p, "insight") != 0)
	...
	default:
	  AllocConsole ();
	  cygwin_attach_handle_to_fd ((char *) "/dev/conin", 0,
				      GetStdHandle (STD_INPUT_HANDLE),
				      1, GENERIC_READ);
	  cygwin_attach_handle_to_fd ((char *) "/dev/conout", 1,
				      GetStdHandle (STD_OUTPUT_HANDLE),
				      0, GENERIC_WRITE);
	  cygwin_attach_handle_to_fd ((char *) "/dev/conout", 2,
