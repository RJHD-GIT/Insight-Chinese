...
# Copyright (C) 1997-2021 Red Hat, Inc.
#
...
proc gdbtk_tcl_preloop { } {
  ...
  if {$gdb_exe_name != ""} {
    # At startup, file_changed_hook is not called, so we must simulate
    # the event by hand.  If the arguments exist
    ...
    gdbtk_tcl_file_changed $gdb_exe_name
    set_baud
  ...
  set onoff off
  if {[pref get gdb/debuginfod]} {
    set onoff on
  }
  catch {gdb_cmd "set debuginfod enabled $onoff"}
...
define_hook debuginfo_progress_hook

# ------------------------------------------------------------------
#  PROCEDURE:  gdbtk_debuginfo_progress - Update debuginfo download progress.
# ------------------------------------------------------------------

proc gdbtk_debuginfo_progress {action {arg ""}} {
  run_hooks debuginfo_progress_hook $action $arg
}
