...
/* User interface output class. This adds specific progress functions to the
   standard cli output. */

class gdbtk_ui_out : public cli_ui_out
{
public:
  explicit gdbtk_ui_out (ui_file *stream, ui_out_flags flags) :
    cli_ui_out (stream, flags)
  {
    m_count = 0;
  }
...
private:
  unsigned int m_count;		/* Number of notify calls. */
};
