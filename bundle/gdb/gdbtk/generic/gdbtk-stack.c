	  ...
	  ALL_BLOCK_SYMBOLS (block, iter, sym)
		...
		case LOC_COMPUTED:	  /* computed location     */
		  Tcl_ListObjAppendElement (interp, result_ptr->obj_ptr,
					    Tcl_NewStringObj (sym->print_name (),
							      -1));
		  break;
	  ...
	  if (flags != 02 /* Not only junk. */ && pc < BLOCK_END (block))
	    {
	      ...
	      addr = string_printf ("%s", paddress (get_current_arch (),
						    BLOCK_START (block)));
	      Tcl_ListObjAppendElement (interp, elt,
					Tcl_NewStringObj (addr.c_str (), -1));
	      addr = string_printf ("%s", paddress (get_current_arch (),
						    BLOCK_END (block)));
	      Tcl_ListObjAppendElement (interp, elt,
					Tcl_NewStringObj (addr.c_str (), -1));
  ...
  while (block != 0)
      ...
      ALL_BLOCK_SYMBOLS (block, iter, sym)
	{
	  ...
	  switch (SYMBOL_CLASS (sym))
	    ...
	    if (arguments == isarg)
		Tcl_ListObjAppendElement (interp, result_ptr->obj_ptr,
					  Tcl_NewStringObj (sym->print_name (), -1));
	}
      if (BLOCK_FUNCTION (block))
	break;
      else
  ...
  if (target_has_registers ())
    {
      const struct block *block;
      block = get_frame_block (get_selected_frame (NULL), 0);
      start = string_printf ("%s", paddress (get_current_arch (),
					     BLOCK_START (block)));
      end = string_printf ("%s", paddress (get_current_arch (),
					   BLOCK_END (block)));
    }

  Tcl_SetListObj (result_ptr->obj_ptr, 0, NULL);
  Tcl_ListObjAppendElement (interp, result_ptr->obj_ptr,
			    Tcl_NewStringObj (start.c_str (), -1));
  Tcl_ListObjAppendElement (interp, result_ptr->obj_ptr,
			    Tcl_NewStringObj (end.c_str (), -1));
  return TCL_OK;
}
		    ...
		    int objc, Tcl_Obj *CONST objv[])
{
  ...
  if (target_has_registers ())
    ...
    frame = string_printf ("%s", paddress (get_current_arch (),
			   get_frame_base (get_selected_frame (NULL))));

  Tcl_SetStringObj (result_ptr->obj_ptr, frame.c_str (), -1);
  return TCL_OK;
}
