...
// Avoid conflict on STRINGIFY macro.
...
static int gdb_prompt_command (ClientData, Tcl_Interp *, int,
			       Tcl_Obj * CONST objv[]);
static int gdb_source_path_command (ClientData, Tcl_Interp *, int,
				    Tcl_Obj * CONST objv[]);
...
static int gdb_set_quit_flag (ClientData, Tcl_Interp *, int, Tcl_Obj * CONST[]);
...
Gdbtk_Init (Tcl_Interp *interp)
{
  ...
  Tcl_CreateObjCommand (interp, "gdb_set_quit_flag", gdbtk_call_wrapper,
			(ClientData) gdb_set_quit_flag, NULL);
  ...
  Tcl_CreateObjCommand (interp, "gdb_source_path", gdbtk_call_wrapper,
			(ClientData) gdb_source_path_command, NULL);
  ...
  if (force)
    {
      ...
      scoped_disable_commit_resumed disable ("gdb_stop");
...
static int
gdb_set_quit_flag (ClientData clientData, Tcl_Interp *interp,
	           int objc, Tcl_Obj *CONST objv[])
{
  set_quit_flag ();
  return TCL_OK;
...
gdb_prompt_command (ClientData clientData, Tcl_Interp *interp,
		    int objc, Tcl_Obj *CONST objv[])
{
  Tcl_SetStringObj (result_ptr->obj_ptr, get_prompt ().c_str (), -1);
  return TCL_OK;
}

/* This implements the tcl command "gdb_source_path"

* It returns the current gdb source path.
*
* Tcl Arguments:
*    None.
* Tcl Result:
...
static int
gdb_source_path_command (ClientData clientData, Tcl_Interp *interp,
			 int objc, Tcl_Obj *CONST objv[])
{
  Tcl_SetStringObj (result_ptr->obj_ptr, source_path.c_str (), -1);
  return TCL_OK;
}

	       ...
	       int objc, Tcl_Obj *CONST objv[])
{
  ...
  for (s = loadfile_bfd->sections; s; s = s->next)
      ...
      if (s->flags & SEC_LOAD)
	{
	  ...
	  if (size > 0)
	    {
	      ob[0] = Tcl_NewStringObj ((char *) bfd_section_name (s), -1);
...
/* An object of this type is passed to map_symbol_filenames.  */
	       ...
	       int objc, Tcl_Obj *CONST objv[])
{
  ...
  map_symbol_filenames (info, false);
	    ...
	    int objc, Tcl_Obj *CONST objv[])
{
  ...
  global_symbol_searcher spec (space, regexp);
  ...
  for (const symbol_search &p : ss)
	...
	continue;
      ...
      if ((p.symbol != NULL
	   && strncmp (p.symbol->linkage_name (), "__tf", 4) != 0
	   && strncmp (p.symbol->linkage_name (), "_GLOBAL_", 8) != 0)
	  || p.msymbol.minsym != NULL)
	{
	  ...
	  if (p.msymbol.minsym == NULL)
	    Tcl_ListObjAppendElement (interp, elem,
				      Tcl_NewStringObj (p.symbol->print_name (),
							-1));
	  else
	    Tcl_ListObjAppendElement (interp, elem,
				      Tcl_NewStringObj (p.msymbol.minsym->print_name (), -1));
	  ...
	  if (SYMBOL_CLASS (sym) == LOC_BLOCK)
	    {
	      ...
	      const char *name = sym->demangled_name ();
		...
		{
		  funcVals[0] = Tcl_NewStringObj (sym->print_name (), -1);
...
gdbtk_load_source (ClientData clientData, struct symtab *symtab,
		   int start_line, int end_line)
{
  struct disassembly_client_data *cli_data =
    (struct disassembly_client_data *) clientData;
  int index_len;

  index_len = Tcl_DStringLength (&cli_data->src_to_line_prefix);

  if (cli_data->file_opened_p == 1)
    {
      ...
      if (!g_source_cache.get_line_charpos (symtab, &offsets))
        return;

      /* First do some sanity checks on the requested lines */

      if (start_line < 1
	  || end_line < start_line || end_line > offsets->size ())
	{
	  return;
	}
      ...
      text_argv = cli_data->source_argv;

      text_argv[3] = line_number;
      text_argv[5] = line;

      if (fseek (cli_data->fp, (*offsets)[start_line - 1], SEEK_SET) < 0)
	{
	  fclose(cli_data->fp);
	  cli_data->file_opened_p = -1;
	  return;
	}

      for (; start_line < end_line; start_line++)
	{
	  if (!fgets (line + 1, 9980, cli_data->fp))
	    {
	      fclose(cli_data->fp);
	      cli_data->file_opened_p = -1;
	      return;
	  ...
	  cli_data->cmd.proc (cli_data->cmd.clientData,
				 cli_data->interp, 7, text_argv);

	  if (*cli_data->map_arr != '\0')
	    {
	      Tcl_DStringAppend (&cli_data->src_to_line_prefix,
				 line_number + 1, -1);

	      Tcl_SetVar2Ex (cli_data->interp, cli_data->map_arr,
			     Tcl_DStringValue (&cli_data->src_to_line_prefix),
			     Tcl_NewIntObj (cli_data->widget_line_no), 0);

	      Tcl_DStringSetLength (&cli_data->src_to_line_prefix, index_len);
	    }
	}

    }
  else if (!cli_data->file_opened_p)
    {
      int fdes;
      ...
      else
	{
	  ...
	  cli_data->file_opened_p = 1;
	  cli_data->fp = fdopen (fdes, FOPEN_RB);
	  clearerr (cli_data->fp);
...
gdb_loc (ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
  ...
  if (objc == 1)
    ...
    {
      std::vector<symtab_and_line> sals;
      const char *lspec = Tcl_GetStringFromObj (objv[1], NULL);

      /* gdb crashes with an empty line spec. */
      if (!lspec || !*lspec)
	{
	  gdbtk_set_result (interp, "Null line spec", -1);
	  return TCL_ERROR;
	}

      sals = decode_line_with_current_source (lspec, 1);
  ...
  if (inferior_ptid != null_ptid && target_has_execution ())
    stop_pc = inferior_thread ()->stop_pc ();
...
/* Covert hex to binary. Stolen from common-utils.cc fromhex
   but changed error handling */
static int
hexdigit (int a)
{
  if (a >= '0' && a <= '9')
		...
		int objc, Tcl_Obj *CONST objv[])
{
  ...
  mptr = cptr = mbuf.data ();
  rnum = target_read (current_inferior ()->top_target (), TARGET_OBJECT_MEMORY,
                      NULL, mbuf.data (), addr, nbytes);
  if (rnum <= 0)
    {
	      ...
	      Tcl_Obj *CONST objv[])
{
  ...
  if (symtab && SYMTAB_OBJFILE (symtab) && SYMTAB_OBJFILE (symtab)->obfd)
    ...
    mtime = bfd_get_mtime(current_program_space->exec_bfd ());
