  ...
  if (target_has_registers ())
    {
      ...
      if (!changed && regstatus == REG_VALID)
        changed = memcmp (regbuf, value_contents_all (val).data (),
			  regsize) != 0;
      if (changed)
        {
          m_register_status[regnum] = regstatus;
          if (regstatus == REG_VALID)
            memcpy (regbuf, value_contents_all (val).data (), regsize);
          else
...
get_register_types (int regnum, map_arg arg)
{
  ...
  if (reg_vtype->code () == TYPE_CODE_UNION)
    {
      n = reg_vtype->num_fields ();
      /* limit to 16 types */
      if (n > 16)
	n = 16;
      ...
      for (i = 0; i < n; i++)
	{
	  struct field &fld = reg_vtype->field (i);
	  Tcl_Obj *ar[3], *list;

	  ar[0] = Tcl_NewStringObj (fld.name (), -1);
	  ar[1] = Tcl_NewStringObj (host_address_to_string (fld.type ()), -1);
	  if (fld.type ()->code () == TYPE_CODE_FLT)
  ...
  else
    {
      ...
      ar[0] = Tcl_NewStringObj (reg_vtype->name (), -1);
      ar[1] = Tcl_NewStringObj (host_address_to_string (reg_vtype), -1);
      if (reg_vtype->code () == TYPE_CODE_FLT)
	ar[2] = Tcl_NewStringObj ("float", -1);
      else
	ar[2] = Tcl_NewStringObj ("int", -1);
      list = Tcl_NewListObj (3, ar);
