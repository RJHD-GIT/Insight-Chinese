...
gdbtk_add_hooks (void)
{
  ...
  gdb::observers::architecture_changed.attach (gdbtk_architecture_changed,
                                               "gdbtk-hooks");
  gdb::observers::memory_changed.attach (gdbtk_memory_changed, "gdbtk-hooks");
  ...
  gdb::observers::register_changed.attach (gdbtk_register_changed,
                                           "gdbtk-hooks");
  gdb::observers::traceframe_changed.attach (gdbtk_trace_find, "gdbtk-hooks");
  gdb::observers::target_pre_wait.attach (gdbtk_target_pre_wait,
                                          "gdbtk-hooks");
...
gdbtk_warning (const char *warning, va_list args)
{
  ...
  if (gdbtk_getpid () != gdbtk_pid)
    ...
    gdbtk_two_elem_cmd ("gdbtk_tcl_warning",
			string_vprintf (warning, args).c_str ());
}
...
gdbtk_ignorable_warning (const char *warnclass, const char *warning)
{
  ...
  if (Tcl_Eval (interp->tcl,
		string_printf ("gdbtk_tcl_ignorable_warning {%s} {%s}",
			       warnclass, warning).c_str ()) != TCL_OK)
    report_error ();
}
...
x_event (int signo)
{
  ...
  /* Also, only do things while the target is running (stops and redraws).
     FIXME: We would like to at least redraw at other times but this is bundled
...
gdbtk_readline_begin (const char *format, ...)
{
  ...
  gdbtk_two_elem_cmd ("gdbtk_tcl_readline_begin",
		      string_vprintf (format, args).c_str ());
  va_end (args);
}
		    ...
		    const char *arg, int from_tty)
{
  ...
  if (cmdblk->theclass == class_run || cmdblk->theclass == class_trace)
	...
	Tcl_Eval (interp->tcl, "gdbtk_tcl_busy");
      ...
      /* We need to allow Tcl event processing in the wait loop below. */
      gdbtk_reenter_tcl ();
...
gdbtk_load_hash (const char *section, unsigned long num)
{
  ...
  if (Tcl_Eval (interp->tcl, string_printf ("Download::download_hash %s %ld",
                                            section, num).c_str ()) != TCL_OK)
    report_error ();
...
void
gdbtk_debuginfo_progress (const char *action, const char *arg)
{
  bool block = gdbtk_block_gdb_events_polling (true);

  try
    {
      gdbtk_two_elem_cmd (string_printf ("gdbtk_debuginfo_progress %s",
					 action).c_str (), arg);
    }
  catch (...)
    {
      gdbtk_block_gdb_events_polling (block);
      throw;
    }
  gdbtk_block_gdb_events_polling (block);
}
...
/* These observer functions are called respectively before and after waiting
   for the target.  */

static void
gdbtk_target_pre_wait (ptid_t ptid)
{
  gdbtk_force_detach = 0;
  gdbtk_start_timer ();
}
...
static void
gdbtk_target_post_wait (ptid_t event_ptid)
{
  gdbtk_stop_timer ();
  gdbtk_ptid = event_ptid;
}
...
gdbtk_query (const char *query, va_list args)
{
  ...
  gdbtk_two_elem_cmd ("gdbtk_tcl_query",
		      string_vprintf (query, args).c_str ());
  return atol (Tcl_GetStringResult (interp->tcl));
}
...
gdbtk_annotate_signal (void)
{
  ...
  if (Tcl_Eval (interp->tcl,
		string_printf ("gdbtk_signal %s {%s}",
			       gdb_signal_to_name (tp->stop_signal ()),
			       gdb_signal_to_string (tp->stop_signal ())).c_str ()) != TCL_OK)
    report_error ();
}
